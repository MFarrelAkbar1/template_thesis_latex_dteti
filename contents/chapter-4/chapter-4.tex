...
\begin{longtable}{|l|p{2.5cm}|p{6cm}|p{3cm}|}
...
($SVR = 27{,}3\%$). Hal tersebut berarti 24 dari 33 blok kode CodeLlama mengandung \textit{parse error}.
...
Gambar \ref{fig:stats-table-a} merangkum statistik lengkap untuk 
kelima model pada Kondisi A.
...
Khusus untuk Qwen2.5-Coder, ketiadaan skor ini tidak
mengindikasikan ketidakmampuan, melainkan mencerminkan
karakteristik respons model yang mengabaikan instruksi format
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{contents/chapter-4/llm_chart_20260427_130844_4_stats_table.png}
...
Gambar~\ref{fig:validation-metrics} menunjukkan dua metrik 
keakuratan yang diukur langsung dari keluaran \textit{pipeline} 
terhadap seluruh 241 \textit{file} hasil konversi, beserta 
profil durasi eksekusi.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{contents/chapter-4/validation/validation_metrics_qwen.png}
    \caption{Metrik akurasi konversi Rector dan durasi eksekusi 
    \textit{pipeline} pada dataset FT UGM}
...
sebagai AI Engine. Durasi ini tidak mencakup eksperimen
...
\end{figure}
\textit{Similarity score} dihitung menggunakan 
\textit{line-level SequenceMatcher} yang mengukur proporsi 
baris yang identik antara dua versi kode. Rata-rata 
\textit{similarity score} adalah 0,9214, dengan distribusi 
yang terkonsentrasi di rentang 0,90--1,00 (11 dari 12 
\textit{file}). Satu \textit{file} memiliki skor di 
rentang 0,70--0,80, yang mencerminkan perbedaan gaya 
...
($CR = 91{,}7\%$), sebagaimana ditunjukkan pada 
Gambar~\ref{fig:rector-bar-dti} dan Gambar~\ref{fig:rector-pie-dti}.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{contents/chapter-4/dti/pipeline_chart_cbt_qwen_2a_rector_bar.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{contents/chapter-4/dti/pipeline_chart_cbt_qwen_2b_rector_pie.png}
    \caption{Proporsi hasil konversi Rector dataset CBT DTI UGM (diagram lingkaran)}
    \label{fig:rector-pie-dti}
\end{figure}
...
\subsubsection{Post-Scan dan AI Engine}
...
Karena terdapat 11 temuan Semgrep, modul AI Engine diaktifkan
...
saran perbaikan dari AI Engine bersifat informatif dan 
...
Hasilnya adalah 2.354 temuan. Di antara temuan tersebut, sekitar 318
...
Status keseluruhan adalah \texttt{NON\_COMPLIANT}. 
Temuan pada A.8.26 dan A.8.29 bersumber dari 
\texttt{system/core/Output.php} milik \textit{framework} 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.98\textwidth]{contents/chapter-4/comparison/comparison_qwen_3_phpstan_iso.png}
...
yang disebabkan oleh \textit{error} PHPStan. Perbedaan utama terletak pada keberadaan temuan Semgrep pada kode
kustom: FT UGM memiliki kerentanan aktif yang memerlukan remediasi
manual, sementara CBT DTI tidak menghasilkan temuan Semgrep di luar
kode \textit{framework}. Dari sisi durasi eksekusi, kedua studi kasus
sebanding, sekitar 3--4 menit, karena keduanya menggunakan
Qwen2.5-Coder 7b sebagai AI Engine dengan satu kali inferensi
