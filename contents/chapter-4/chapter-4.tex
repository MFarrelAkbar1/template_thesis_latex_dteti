...
(\textit{Server-Side Request Forgery}). Ini adalah seluruh populasi 
...
{\footnotesize \texttt{NON\_COMPLIANT}} \\
\end{longtable}
Status keseluruhan adalah \texttt{NON\_COMPLIANT}. Ini bukan hasil yang mengejutkan mengingat kondisi kode warisan yang dianalisis; justru ini adalah representasi nyata tentang postur keamanan aplikasi sebelum dilakukan remediasi manual.
...
Khusus untuk Qwen2.5-Coder, ketiadaan skor ini bukan tanda 
ketidakmampuan, melainkan refleksi gaya respons model yang 
...
yang tertinggi dibanding model lain, selaras dengan respons yang 
...
lolos pemeriksaan tanpa \textit{parse error} (100\%). Ini 
...
perbandingan dilakukan antara keluaran Rector dan referensi 
LLM yang dihasilkan oleh \textit{claude-sonnet-4-20250514} 
via Anthropic API, hanya 12 \textit{file} dari 
...
dan \textit{deprecated removed} 100\%. Ini mengkonfirmasi 
...
\end{figure}
\newpage
Tingkat konversi yang lebih rendah dibanding studi kasus FT UGM 
...
Ini konsisten dengan cakupan analisis \textit{full scope} 
...
Injection dan 10 SSRF pada kedua dataset. Ini bukan kebetulan, 
melainkan mencerminkan fakta bahwa kedua aplikasi menggunakan 
