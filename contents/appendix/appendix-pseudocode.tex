...
% Style khusus pseudocode: columns=fullflexible + breakautoindent=false
% menghilangkan Overfull \hbox yang terjadi pada columns=flexible
...
// Cocokkan ID aturan dan pesan ke dalam VULNERABILITY_RULES
  ...
  FOR each (keywords, label, controls, priority) in VULNERABILITY_RULES DO
    ...
    RETURN ("Other", ["A.8.28"], priority=99)  // kategori tidak dikenal
END FUNCTION
\end{lstlisting}
...
FUNCTION run_conversion(input_path, output_path, target_version):
    ...
    // Step 2: Pilih rantai aturan Rector sesuai versi PHP yang ditemukan
    ...
    // Step 3: Salin input/ ke output/ agar input asli tidak pernah diubah
    ...
    // Step 5: Tulis konfigurasi Rector sementara ke direktori temp sistem
    ...
    FOR each source_file in php_files DO
        ...
        ELSE
            status <- SKIPPED    // tidak ada perubahan yang perlu diterapkan
        END IF
...
DATA STRUCTURE AnalysisResult:
    errors     : list of AnalysisError  // diurutkan per file lalu per baris
...
FUNCTION classify_error(message):
    ...
    RETURN ("ERROR", ["A.8.28"])   // fallback untuk pesan yang tidak dikenal
END FUNCTION
\end{lstlisting}
...
FUNCTION parse_response(raw_text, original_code, iso_controls, ...):
    ...
    // Coba tag XML <explanation>, lalu "1. EXPLANATION:", lalu teks bebas

    // --- Ekstraksi kode perbaikan ---
    // Coba tag XML <fix>, lalu "2. FIXED CODE:", lalu blok ```php ... ```
    ...
    FOR each model in models DO
        ...
        FOR each finding in eligible DO
            IF prompt_mode == "standard" THEN
                messages <- build_chat_messages(finding)   // Kondisi A
            ELSE
                // Kondisi B: prompt dioptimalkan per model
                messages <- build_optimized_chat_messages(finding, model)
            END IF
...
CONSTANT CONTROL_REGISTRY:
    ...
    A.8.26  Application Security Requirements (persyaratan keamanan aplikasi)
...
FUNCTION run_iso_mapping(scan_result, analysis_result, ai_recs, output_path):
    ...
    IF scan_result != NULL THEN
        FOR each finding in scan_result.findings DO
            // INFO dinaikkan ke WARNING agar tidak ada status COMPLIANT palsu
    ...
    FOR each rec in ai_recs DO
        evidence <- "[AI/model][vuln_type] file:baris -- fix (confidence)"
    ...
    // Step 6: Status keseluruhan = status terburuk di antara semua kontrol
