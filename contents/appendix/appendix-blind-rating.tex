...
769  {
770 >> log_message('error', 'Unable to delete cache file for 
       '.$uri);
771     return FALSE;
772  }
773
...
public function delete_cache($uri = '')
{
    ...
    if (!is_dir($cache_path)) {
        log_message('error', 'Unable to find cache path: 
        '.$cache_path);
        return FALSE;
    }
    if (empty($uri)) {
        $uri = $CI->uri->uri_string();
        if (($cache_query_string = $CI->config->
        item('cache_query_string'))
...
583  {
584 >> log_message('error', 'Unable to write cache file: 
    '.$cache_path);
585     return;
586  }
587
