    ...
    Pendekatan ini terbukti efektif: pengujian pada phpBB versi 2.0.0 (37.193 baris kode PHP 4.2.2) berhasil menghentikan semua 176 serangan injeksi yang diuji tanpa satu pun \textit{false positive}. Namun, pendekatan ini memiliki keterbatasan. Kualitas perlindungan sepenuhnya bergantung pada kelengkapan rangkaian pengujian yang digunakan selama fase analisis dinamis. Titik akses yang tidak tercakup pengujian tidak terlindungi.
    ...
    Li dkk. \cite{li2025software} mengambil pendekatan yang lebih terstruktur. Mereka menggabungkan analisis \textit{patch} historis dengan analisis statis, lalu menambahkan tabel fitur kerentanan dan templat penganalisis untuk membuat klasifikasi LLM lebih akurat, terutama pada kerentanan yang membutuhkan pemahaman aliran data lintas fungsi. Evaluasi pada basis kode Linux kernel berhasil menemukan 44 bug baru, 37 di antaranya dikonfirmasi dan 30 diperbaiki, dengan hasil yang lebih baik dibandingkan analisis statis saja.

    Terdapat dua temuan penting yang dapat ditarik dari kedua studi tersebut. LLM tujuan umum sudah bisa menemukan pola kerentanan hanya dengan \textit{prompt engineering}, dan penggabungan LLM dengan analisis statis terstruktur memberikan hasil yang lebih akurat untuk kasus yang kompleks. Namun, hambatan yang sama muncul di keduanya: model dijalankan via API \textit{cloud}, yang menjadi kendala nyata ketika kode yang dianalisis bersifat rahasia. Penelitian ini mengatasi kendala tersebut dengan menjalankan semua model secara lokal melalui Ollama, sehingga tidak ada kode sumber yang keluar dari lingkungan organisasi.
    ...
    Baik St\"{u}mpfle dkk. maupun penelitian ini menggunakan LLM untuk transformasi kode yang melampaui substitusi sintaksis dan membutuhkan pemahaman semantik. \textit{Loop} umpan balik dalam St\"{u}mpfle dkk. memiliki fungsi konseptual yang mirip dengan peran validasi PHPStan dalam \textit{pipeline} penelitian ini: keduanya menggunakan \textit{output} alat analisis statis untuk memverifikasi apakah transformasi berhasil. Perbedaan utamanya terletak pada fokus: St\"{u}mpfle dkk. menangani ekstraksi fitur lintas varian untuk rekayasa lini produk, sedangkan penelitian ini berfokus pada migrasi versi dan verifikasi kepatuhan keamanan untuk kode warisan dalam satu bahasa.
    ...
    Strobl dkk. \cite{strobl2020automated} menganalisis tiga kasus transformasi kode otomatis skala besar di sebuah organisasi pemerintah, semuanya melibatkan konversi sistem warisan dari COBOL dan PL/I ke Java dengan ukuran lebih dari satu juta baris kode. Fokus utama studi ini bukan pada keberhasilan transformasi, melainkan pada dampak pascatransformasi.

    Hasil studi tersebut mengungkapkan permasalahan yang signifikan. Transformasi otomatis seringkali menghasilkan kode yang sintaksis valid di bahasa target, tapi mempertahankan karakteristik struktural dari bahasa sumber sehingga sulit dipahami pengembang yang tidak kenal sistem lama. Lebih lanjut, sistem yang memiliki \textit{test suite} otomatis sejak awal jauh lebih mudah dirawat pascatransformasi, sementara sistem tanpa pengujian otomatis menjadi beban jangka panjang. (Menggabungkan dua kalimat pendek menjadi satu kalimat majemuk yang lebih elegan, serta mengganti kata "kasus" menjadi "sistem").
  ...
  \subsection{Migrasi Kode Berbasis AST di Lingkungan Produksi}
    ...
    \hline
    \endlastfoot
    Merlo dkk. \cite{merlo2007automated} & PHP & Tidak ada & \textit{Security remediation} \\
    \hline
    Akuthota dkk. \cite{akuthota2023vulnerability} & Java & GPT-3.5-Turbo & \textit{Vulnerability detection} \\
    \hline
    Li dkk. \cite{li2025software} & C (Linux kernel) & LLM + analisis statis & \textit{Vulnerability detection} \\
    \hline
    St\"{u}mpfle dkk. \cite{stumpfle2025llm} & C/C++ & LLM & \textit{Code transformation} \\
    \hline
    Li, Dutta \& Naik \cite{li2025iris} & Java & GPT-4 & \textit{Vulnerability detection} \\
    \hline
    Strobl dkk. \cite{strobl2020automated} & COBOL/PL/I & Tidak ada & \textit{Code transformation} \\
    \hline
    Cai dkk. \cite{cai2015pattern} & Java & Tidak ada & \textit{Cloud migration} \\
    \hline
    Chen dkk. \cite{chen2022ast} & TypeScript & Tidak ada & \textit{Code migration} (AST) \\
    \hline
    \textbf{Penelitian ini} & PHP & Membandingkan 5 LLM lokal dan memilih AI terbaik untuk migrasi PHP & \textit{Version migration} + \textit{security compliance} \\
    \hline
    \end{longtable}
    ...
    \hline
    \endlastfoot
    Merlo dkk. \cite{merlo2007automated} & N/A & Tidak & Tidak ada & Ya \\
    \hline
    Akuthota dkk. \cite{akuthota2023vulnerability} & \textit{Cloud} & Tidak & Tidak ada & Tidak \\
    \hline
    Li dkk. \cite{li2025software} & \textit{Cloud} & Tidak & Tidak ada & Tidak \\
    \hline
    St\"{u}mpfle dkk. \cite{stumpfle2025llm} & \textit{Cloud} & Ya (\textit{feedback loop}) & Tidak ada & Tidak \\
    \hline
    Li, Dutta \& Naik \cite{li2025iris} & \textit{Cloud} & Tidak & Tidak ada & Ya (IRIS) \\
    \hline
    Strobl dkk. \cite{strobl2020automated} & N/A & Tidak & Tidak ada & Tidak \\
    \hline
    Cai dkk. \cite{cai2015pattern} & N/A & Tidak & Tidak ada & Tidak \\
    \hline
    Chen dkk. \cite{chen2022ast} & N/A & Tidak & Tidak ada & Tidak \\
    \hline
    \textbf{Penelitian ini} & Lokal (Ollama) & Ya (PHPStan + Semgrep) & ISO/IEC 27001:2022 & Ya \\
    \hline
    \end{longtable}

    Kedua tabel memperlihatkan kesenjangan yang konsisten. Studi-studi yang berfokus pada keamanan tidak menangani migrasi versi. Studi-studi yang berfokus pada transformasi kode, termasuk yang sudah terbukti di skala industri seperti Chen dkk. \cite{chen2022ast} tidak menyertakan pemeriksaan keamanan maupun pemetaan ke standar formal. Tidak satu pun dari kedelapan studi yang menggabungkan migrasi versi PHP, analisis keamanan berbasis LLM, validasi pasca-konversi, dan kepatuhan ISO/IEC 27001:2022 dalam satu alur kerja \textit{open source} dengan \textit{deployment} lokal.

    Penting untuk digarisbawahi bahwa kedua tabel di atas membandingkan kehadiran fitur (\textit{feature presence}), bukan performa pada tugas yang sama. Kedelapan studi pembanding menyasar masalah yang secara fundamental berbeda dari penelitian ini -- bahasa target, jenis kerentanan, dan tujuan transformasi yang berbeda -- sehingga tidak dapat dibandingkan secara \textit{apple-to-apple} dalam metrik kinerja apa pun, dan tabel ini tidak dimaksudkan sebagai klaim bahwa penelitian ini mengungguli studi-studi tersebut. Perbedaan paling mendasar antara penelitian ini dan kedelapan studi tersebut juga bukan pada cakupan fitur yang lebih banyak, melainkan pada satu kemampuan spesifik yang belum dieksplorasi pada konteks PHP: menggunakan LLM lokal untuk menerjemahkan temuan mentah Semgrep menjadi penjelasan dan saran remediasi yang dapat diaudit, tanpa mengubah peran Semgrep sebagai detektor maupun Rector sebagai mesin transformasi. Rector, Semgrep, dan PHPStan sendiri bukan kontribusi baru dari penelitian ini; ketiganya digunakan sebagaimana adanya. Kombinasi penempatan lapisan AI di antara ketiganya, dengan validasi pasca-konversi dan pemetaan ke ISO/IEC 27001:2022 yang melekat pada alur kerja, adalah kontribusi yang diklaim penelitian ini, dan klaim tersebut diuji secara empiris melalui implementasi pada Bab IV, bukan diasumsikan dari tabel perbandingan ini.

    Secara keseluruhan, tinjauan literatur menunjukkan bahwa perkembangan riset pada ketiga bidang tersebut masih berjalan secara terpisah. Pada sisi remediasi keamanan kode PHP, Merlo dkk. \cite{merlo2007automated} sudah membuktikan kelayakan otomatisasi sejak 2007, namun solusi yang dihasilkan tidak pernah diperluas untuk menangani migrasi versi maupun standar kepatuhan formal. Pada sisi deteksi kerentanan berbasis LLM, kemajuan dari \textit{prompt engineering} murni (Akuthota dkk. \cite{akuthota2023vulnerability}) ke kombinasi LLM dan analisis statis yang terstruktur (Li dkk. \cite{li2025software}; Li, Dutta, dan Naik \cite{li2025iris}) konsisten menunjukkan bahwa LLM paling efektif ketika berperan sebagai lapisan interpretasi di atas alat analisis statis yang sudah ada, bukan sebagai pengganti alat tersebut -- temuan yang menjadi dasar konseptual langsung bagi peran AI Engine dalam penelitian ini. Pada sisi transformasi kode, kemajuan dari transformasi berbasis pola tekstual (Cai dkk. \cite{cai2015pattern}) ke transformasi berbasis AST yang tervalidasi di skala produksi (Chen dkk. \cite{chen2022ast}) dan transformasi berbantuan LLM dengan \textit{loop} umpan balik (St\"{u}mpfle dkk. \cite{stumpfle2025llm}) menunjukkan bahwa pendekatan struktural berbasis AST untuk perubahan sintaksis dan LLM untuk perubahan semantik adalah dua kemampuan yang saling melengkapi, bukan saling menggantikan.

    Kesenjangan penelitian yang teridentifikasi adalah belum adanya integrasi dari ketiga pendekatan tersebut untuk menangani kasus migrasi versi aplikasi PHP lawas. Tidak satu pun studi yang ditinjau menggabungkan transformasi kode berbasis AST, deteksi kerentanan berbasis aturan, lapisan interpretasi berbasis LLM lokal, dan pemetaan ke standar kepatuhan formal dalam satu alur kerja yang dapat diaudit. Penelitian ini berkontribusi secara spesifik dan terbatas pada interseksi tersebut: bukan mengusulkan alat transformasi baru karena Rector sudah memadai untuk itu, dan bukan pula mengusulkan detektor kerentanan baru karena Semgrep sudah memadai untuk itu, melainkan menambahkan lapisan AI yang menerjemahkan temuan Semgrep menjadi penjelasan dan saran remediasi yang dipetakan ke kontrol ISO/IEC 27001:2022, sekaligus mengevaluasi lima kandidat model LLM lokal untuk menentukan mana yang paling sesuai menjalankan peran tersebut.

    Kemajuan terkini pada ketiga bidang yang ditinjau menunjukkan bahwa 
kombinasi LLM dengan alat analisis statis adalah arah yang paling 
menjanjikan: IRIS \cite{li2025iris} membuktikan peningkatan deteksi 
kerentanan sebesar dua kali lipat dibanding analisis statis saja, 
sementara St\"{u}mpfle dkk.\ \cite{stumpfle2025llm} menunjukkan 
transformasi kode berbantuan LLM dapat mengurangi kegagalan akibat 
halusinasi secara signifikan melalui \textit{feedback loop}. Namun, 
kesenjangan yang belum tertangani adalah penerapan kombinasi ini pada 
konteks migrasi versi PHP dengan kendala privasi data, di mana kode 
sumber tidak dapat dikirim ke layanan \textit{cloud}. Penelitian ini 
mengisi kesenjangan tersebut dengan membangun \textit{pipeline} yang 
menempatkan LLM lokal sebagai lapisan interpretasi di atas Semgrep 
dan Rector, sekaligus memetakan hasilnya ke kontrol ISO/IEC 27001:2022 
dalam satu alur kerja yang dapat diaudit.
    \subsection{Pertanyaan Penelitian}

Berdasarkan kesenjangan yang teridentifikasi, penelitian ini mengajukan 
empat pertanyaan penelitian berikut:

\begin{enumerate}
    \item Apakah \textit{pipeline} yang dibangun berhasil memigrasikan 
    aplikasi web PHP 7.x ke PHP 8.3 tanpa menimbulkan regresi fungsional 
    pada kode sumber yang dikonversi?
    \item Apakah hasil konversi mempertahankan fungsionalitas kode sumber 
    asli sebagaimana dapat diverifikasi melalui pemeriksaan validitas 
    sintaksis dan kompatibilitas tipe pasca-konversi?
    ...
    keamanan tidak lagi memerlukan penerjemahan manual menjadi bukti audit?
    \item Apakah \textit{AI Engine} berbasis LLM \textit{open source} 
    lokal yang diusulkan menghasilkan penjelasan dan saran remediasi yang 
    secara terukur lebih informatif dibandingkan keluaran \textit{pipeline} 
    tanpa komponen AI?
\end{enumerate}
    ...
    PHP (\textit{Hypertext Preprocessor}) telah menjadi bahasa skrip sisi \textit{server} yang paling banyak digunakan di web sejak akhir 1990-an \cite{phpgroup_history}. W3Techs mencatat bahwa 71,7\% dari seluruh situs web yang memiliki bahasa pemrograman sisi \textit{server} menggunakan PHP \cite{w3techs2024php}.
...
Dalam praktiknya, DevSecOps diwujudkan melalui integrasi alat keamanan otomatis ke dalam \textit{pipeline}: analisis statis kode sumber (SAST), pemindaian dependensi, dan pengujian keamanan dinamis. Kerentanan yang ditemukan lebih awal jauh lebih murah untuk diperbaiki dibandingkan yang baru ditemukan setelah sistem berjalan di produksi.
    ...
    Penelitian ini mengevaluasi lima model LLM \textit{open source} yang dijalankan secara lokal. Kriteria pemilihan ada dua: model harus bisa dijalankan di GPU dengan VRAM 8 GB menggunakan kuantisasi Q4\_0, dan harus memiliki kemampuan pemahaman kode yang terdokumentasi dalam literatur ilmiah.
    ...
    Penelitian ini mengadaptasi kedua konsep tersebut ke dalam lima metrik
    ...
    \subsection{Algoritma Ratcliff/Obershelp untuk Pengukuran \textit{Similarity} Kode}
\label{subsec:ratcliff-obershelp}

Pengukuran kemiripan antara dua versi kode sumber memerlukan algoritma yang sensitif 
terhadap urutan dan struktur baris, bukan sekadar kehadiran kata kunci. Algoritma 
Ratcliff/Obershelp \cite{ratcliff1988gestalt} dirancang untuk kebutuhan ini. Berbeda 
dari \textit{cosine similarity} yang merepresentasikan teks sebagai vektor frekuensi 
kata dan mengabaikan urutan, atau \textit{edit distance} (Levenshtein) yang bekerja 
pada level karakter, Ratcliff/Obershelp menghitung rasio kecocokan berbasis blok 
terpanjang yang cocok secara rekursif antara dua sekuens.
...
di mana $M$ adalah jumlah elemen yang cocok dan $T$ adalah total elemen pada kedua 
sekuens. Nilai $S$ berada di rentang $0{,}0$ (tidak ada kesamaan) hingga $1{,}0$ 
(identik). Dalam konteks perbandingan kode sumber, algoritma ini diterapkan pada 
level baris (\textit{line-level}), sehingga setiap baris kode diperlakukan sebagai 
satu elemen sekuens.

Python mengimplementasikan algoritma ini melalui \texttt{difflib.SequenceMatcher}
dari \textit{standard library}, yang menjadikannya dapat digunakan tanpa dependensi
eksternal tambahan. Dalam penelitian ini, metrik ini digunakan pada tahap validasi
akurasi untuk mengukur kemiripan antara keluaran Rector dan konversi referensi yang
disiapkan secara manual, sebagaimana dibahas di Subbab \ref{subsec:validasi-akurasi}.

\subsection{Evaluasi Berbasis Penilaian Manusia}
\label{subsec:evaluasi-manusia}

Skala Likert \cite{likert1932technique} adalah skala penilaian ordinal yang
digunakan untuk mengukur sikap atau penilaian subjektif terhadap suatu objek
melalui serangkaian poin bertingkat, biasanya dari sangat tidak setuju hingga
sangat setuju. Skala ini banyak digunakan dalam penelitian yang melibatkan
penilaian kualitas oleh manusia karena sederhana diterapkan namun tetap
memberikan gradasi penilaian yang lebih informatif dibandingkan skala biner.

\textit{Wilcoxon signed-rank test} \cite{wilcoxon1945individual} adalah uji
statistik nonparametrik untuk membandingkan dua kondisi berpasangan pada data
berskala ordinal. Uji ini dipilih ketika asumsi normalitas data tidak dapat
dipenuhi, sebagaimana umum terjadi pada sampel berukuran kecil atau data hasil
penilaian skala Likert, sehingga lebih sesuai dibandingkan uji parametrik
seperti \textit{paired t-test}.

\textit{Cohen's kappa} \cite{cohen1960coefficient} adalah ukuran reliabilitas
antar-\textit{rater} (\textit{inter-rater reliability}) yang mengukur tingkat
kesepakatan antara dua \textit{rater} setelah memperhitungkan kesepakatan yang
mungkin terjadi secara kebetulan. Nilai ini penting untuk memastikan bahwa
penilaian yang diberikan \textit{rater} cukup konsisten sebelum hasilnya
