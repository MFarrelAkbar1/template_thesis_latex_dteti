  ...
  \subsection{Migrasi Kode Berbasis AST di Lingkungan Produksi}
    ...
    Signifikansi studi ini terletak pada validasi teknis pendekatannya di lingkungan produksi nyata, bukan pada skala operasi. Chen et al. mendemonstrasikan bahwa transformasi berbasis AST dapat diterapkan secara otomatis pada basis kode TypeScript di Microsoft dan menghasilkan perubahan yang struktural konsisten, mengkonfirmasi kelayakan teknis pendekatan serupa yang mendasari Rector untuk migrasi kode dalam konteks institusional.
    ...
    \hline
    Akuthota et al. \cite{akuthota2023vulnerability} & Java & GPT-3.5-Turbo & \textit{Vulnerability detection} \\
    \hline
    ...
    Ditinjau secara keseluruhan, kemajuan riset pada tiga arah yang dibahas di atas berjalan paralel tanpa pernah bertemu. Pada sisi remediasi keamanan kode PHP, Merlo dkk. \cite{merlo2007automated} sudah membuktikan kelayakan otomatisasi sejak 2007, namun solusi yang dihasilkan tidak pernah diperluas untuk menangani migrasi versi maupun standar kepatuhan formal. Pada sisi deteksi kerentanan berbasis LLM, kemajuan dari \textit{prompt engineering} murni (Akuthota dkk. \cite{akuthota2023vulnerability}) ke kombinasi LLM dan analisis statis yang terstruktur (Li et al. \cite{li2025software}; Li, Dutta, dan Naik \cite{li2025iris}) konsisten menunjukkan bahwa LLM paling efektif ketika berperan sebagai lapisan interpretasi di atas alat analisis statis yang sudah ada, bukan sebagai pengganti alat tersebut -- temuan yang menjadi dasar konseptual langsung bagi peran AI Engine dalam penelitian ini. Pada sisi transformasi kode, kemajuan dari transformasi berbasis pola tekstual (Cai et al. \cite{cai2015pattern}) ke transformasi berbasis AST yang tervalidasi di skala produksi (Chen et al. \cite{chen2022ast}) dan transformasi berbantuan LLM dengan \textit{loop} umpan balik (St\"{u}mpfle dkk. \cite{stumpfle2025llm}) menunjukkan bahwa pendekatan struktural berbasis AST untuk perubahan sintaksis dan LLM untuk perubahan semantik adalah dua kemampuan yang saling melengkapi, bukan saling menggantikan.

    \textit{Gap} penelitian yang tersisa dari kemajuan-kemajuan ini bukan pada masing-masing arah secara terpisah, melainkan pada ketidakhadiran integrasi ketiganya untuk kasus migrasi PHP lawas. Tidak satu pun studi yang ditinjau menggabungkan transformasi kode berbasis AST, deteksi kerentanan berbasis aturan, lapisan interpretasi berbasis LLM lokal, dan pemetaan ke standar kepatuhan formal dalam satu alur kerja yang dapat diaudit. Penelitian ini berkontribusi secara spesifik dan terbatas pada interseksi tersebut: bukan mengusulkan alat transformasi baru karena Rector sudah memadai untuk itu, dan bukan pula mengusulkan detektor kerentanan baru karena Semgrep sudah memadai untuk itu, melainkan menambahkan lapisan AI yang menerjemahkan temuan Semgrep menjadi penjelasan dan saran remediasi yang dipetakan ke kontrol ISO/IEC 27001:2022, sekaligus mengevaluasi lima kandidat model LLM lokal untuk menentukan mana yang paling sesuai menjalankan peran tersebut.
...
Rumusan masalah pada Subbab~\ref{sec:rumusan-masalah} menetapkan akar permasalahan yang melatari penelitian ini, dan tinjauan literatur serta dasar teori pada bab ini menetapkan posisi \textit{platform} yang diusulkan terhadap kesenjangan yang ada. Berdasarkan keduanya, penelitian ini mengajukan empat pertanyaan penelitian yang menguji apakah solusi yang diusulkan -- \textit{pipeline} yang menggabungkan Rector, Semgrep, PHPStan, dan lapisan AI Engine berbasis LLM lokal -- dapat menyelesaikan permasalahan yang diangkat. Berbeda dari rumusan masalah yang menyatakan persoalan yang harus diselesaikan, pertanyaan-pertanyaan ini diajukan dalam bentuk yang dapat diuji secara empiris melalui hasil implementasi pada Bab IV:

\begin{enumerate}
    \item Apakah \textit{pipeline} berbasis AI yang dirancang mampu
    ...
    \item Apakah hasil konversi yang dihasilkan \textit{pipeline}
    terbukti, melalui pemeriksaan validitas sintaksis dan kompatibilitas
    tipe pasca-konversi, mempertahankan fungsionalitas kode sumber yang
    telah dikonversi?
    \item Apakah pemeriksaan \textit{secure coding} yang diintegrasikan
    ke dalam \textit{pipeline} konversi otomatis selaras dengan kontrol
    ISO/IEC 27001:2022 Annex A.8, sehingga temuan keamanan dapat
    diterjemahkan menjadi bukti audit tanpa pekerjaan manual yang
    terpisah dari proses migrasi?
    \item Model LLM \textit{open source} lokal manakah yang memberikan
    analisis keamanan dan saran perbaikan kode PHP terbaik dalam
    konteks migrasi dari PHP 7.x ke PHP 8.3?
\end{enumerate}

Jawaban atas keempat pertanyaan ini, beserta bukti empiris yang mendasarinya, dibahas pada Bab IV dan digunakan untuk menyusun kesimpulan pada Bab V.