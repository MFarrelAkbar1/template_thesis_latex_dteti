    ...
    Karya ini relevan bukan karena pendekatan teknisnya, melainkan karena membuktikan bahwa analisis keamanan dan perbaikan kode PHP skala besar sangat mungkin diotomatisasi. Mengingat studi ini terbit hampir dua dekade lalu namun solusi komprehensif berstandar modern belum juga tersedia, jelas ada celah penelitian yang harus segera diisi.
    ...
    Hasil studi tersebut mengungkapkan permasalahan yang signifikan. 
Transformasi otomatis seringkali menghasilkan kode yang sintaksis 
valid di bahasa target, tapi mempertahankan karakteristik struktural 
dari bahasa sumber sehingga sulit dipahami pengembang yang tidak 
kenal sistem lama. Lebih lanjut, sistem yang memiliki \textit{test 
suite} otomatis sejak awal jauh lebih mudah dirawat pascatransformasi, 
sementara sistem tanpa pengujian otomatis menjadi beban jangka panjang.
  ...
  \subsection{Migrasi Kode Berbasis AST di Lingkungan Produksi}
    ...
    Kedua tabel di atas membandingkan kehadiran fitur (\textit{feature presence}), bukan performa absolut. Mengingat kedelapan studi pembanding menyasar bahasa target dan jenis kerentanan yang berbeda, komparasi kinerja secara langsung tidak dapat dilakukan. Kontribusi utama penelitian ini tidak sekadar memperbanyak cakupan fitur, melainkan mengintegrasikan kemampuan spesifik yang belum dieksplorasi pada konteks PHP: menggunakan LLM lokal untuk menerjemahkan temuan mentah Semgrep menjadi penjelasan dan saran remediasi yang dapat diaudit, tanpa mengubah peran Semgrep sebagai detektor maupun Rector sebagai mesin transformasi. Rector, Semgrep, dan PHPStan sendiri bukan kontribusi baru dari penelitian ini; ketiganya digunakan sebagaimana adanya. Kombinasi penempatan lapisan AI di antara ketiganya, dengan validasi pasca-konversi dan pemetaan ke ISO/IEC 27001:2022 yang melekat pada alur kerja, adalah kontribusi yang diklaim penelitian ini, dan klaim tersebut diuji secara empiris melalui implementasi pada Bab IV, bukan diasumsikan dari tabel perbandingan ini.

    Berdasarkan tinjauan literatur di atas, terlihat jelas bahwa riset mengenai perbaikan keamanan, deteksi kerentanan berbasis LLM, dan transformasi kode masih berkembang secara terpisah. Kemajuan terkini menunjukkan bahwa kombinasi LLM dengan alat analisis statis adalah arah yang paling menjanjikan, sebagaimana dibuktikan oleh IRIS \cite{li2025iris} dan St"{u}mpfle dkk. \cite{stumpfle2025llm}. Kesenjangan penelitian yang belum tertangani adalah penerapan kombinasi ini pada konteks migrasi versi PHP dengan kendala privasi data. Penelitian ini mengisi kesenjangan tersebut dengan membangun \textit{pipeline} yang menempatkan LLM lokal sebagai lapisan interpretasi di atas Semgrep dan Rector, serta memetakannya ke standar kepatuhan formal ISO/IEC 27001:2022 dalam satu alur kerja yang dapat diaudit.
